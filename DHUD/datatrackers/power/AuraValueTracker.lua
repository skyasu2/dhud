...
function DHUDAuraValueTracker:updateAuraValue()
	...
	for i, v in ipairs(self.aurasToTrack) do
		local nameToFind = trackingHelper:getSpellName(v);
		-- iterate helpful auras
		for j = 1, 40 do
			local aura = C_UnitAuras.GetAuraDataByIndex(self.unitId, j, "HELPFUL");
			if (aura == nil) then
				break;
			end
			if (aura.name == nameToFind) then
				if (aura.points) then
					auraValue = aura.points[1];
				end
				break;
			end
		end
