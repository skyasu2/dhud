...
-- Bar style metadata (id matches texture file suffix; index must equal id)
Textures.BAR_STYLES = {
    [2] = { id = 2, name = "Classic Bold" },
}

-- Fonts
local defaultFontPath = "Fonts\\FRIZQT__.TTF"
if _G.GameFontNormal then
    local path = _G.GameFontNormal:GetFont()
    if path then defaultFontPath = path end
end

local locale = GetLocale and GetLocale() or ""

-- Use locale-safe numeric font for locales where the addon numeric font
-- may lack Latin/Hangul glyphs (e.g., koKR). Fallback to default UI font.
local numericFont = ART .. "Number.TTF"
if locale == "koKR" then
    numericFont = defaultFontPath
end

Textures.fonts = {
    default = defaultFontPath,
    numeric = numericFont,
}
