...
function UnitInfoSlot:OnDataChanged()
    ...
    -- name can be secret (UnitName in Midnight); only compare when non-secret
    local nameIsSecret = issecretvalue and issecretvalue(t.name)
    if not nameIsSecret and t.name == "" then
    ...
    if self.textField2 then
        ...
        elseif (issecretvalue and issecretvalue(t.creatureType)) or t.creatureType ~= "" then
