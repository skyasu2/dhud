...
local function SetupBarSlot(slotName)
    ...
    local pctTextFrame = Layout:GetPctTextFrame(slotName)
    ...
    renderer:SetTextFrame(textFrame)
    renderer:SetPctTextFrame(pctTextFrame)

    -- Fixed text Y positions below bar bottom
    -- Combined "VALUE PCT%" in textField; pctField used only for percent-only mode
    --   Y = -20  Big2 text (fixed)
    --   Y = -34  Big1 text (fixed)
    if slotName:find("Big2") then
        renderer.pctFixedY = -20
    elseif slotName:find("Big1") then
        renderer.pctFixedY = -34
    end

    local slot = ns.BarSlot:New(slotName, info.barType)
    slot:Init(renderer, textFrame, pctTextFrame, info.unitId)
...
function HUDManager:Init()
    ...
    TrackerHelper.events:On("PetChanged", self, self.OnPetChanged)
    ...
    -- Refresh bar textures (both original and StatusBar) when style changes
    Settings:OnChange("barsTexture", self, function()
        Layout:RefreshBarStyles()
        for _, slot in pairs(barSlots) do
            if slot.renderer and slot.renderer.RefreshStatusBarTexture then
                slot.renderer:RefreshStatusBarTexture()
            end
        end
    end)
...
function HUDManager:Cleanup()
    ...
    TrackerHelper.events:Off("PetChanged", self, self.OnPetChanged)
    ...
    if not TrackerHelper.isTargetAvailable then
        for slotName, slot in pairs(barSlots) do
            if slot.unitId == "target" then
                ...
                if slot.pctField then slot.pctField:DSetText("") end
            end
        end
    end
    Layout:RefreshBackgrounds()
...
function HUDManager:OnPetChanged()
    for key, tracker in pairs(trackers) do
        if tracker.unitId == "pet" then
            if TrackerHelper.isPetAvailable then
                if tracker.StartTracking and not tracker.isTracking then
                    tracker:StartTracking()
                end
                if tracker.UpdateAllData then
    ...
    -- Hide pet bar slots when pet dismissed
    if not TrackerHelper.isPetAvailable then
        for slotName, slot in pairs(barSlots) do
            if slot.unitId == "pet" then
                if slot.renderer then slot.renderer:HideBar() end
                if slot.textField then slot.textField:DSetText("") end
                if slot.pctField then slot.pctField:DSetText("") end
            end
...
local function IsTargetInRange()
    ...
    if issecretvalue and issecretvalue(ok) then return true end
...
function HUDManager:OnRangeUpdate()
    ...
    local a = inRange and 1 or 0.35
    Layout:SetSideAlpha("right", a)
end

-- Force refresh all active bar slots (e.g. after visual setting change)
function HUDManager:ForceRefreshSlots()
    for _, slot in pairs(barSlots) do
        if slot.isActive and slot.OnDataChanged then
            slot:OnDataChanged()
        end
    end
end
...
function HUDManager:RebuildSlot(slotName)
    ...
    if slot then
        ...
        local pctTextFrame = Layout:GetPctTextFrame(slotName)
        if pctTextFrame and pctTextFrame.textField and pctTextFrame.textField.DSetText then pctTextFrame.textField:DSetText("") end
    end
end
